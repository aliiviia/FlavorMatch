...
import aiFormatRecipe from "./routes/aiFormatRecipe.js";

dotenv.config();
const app = express();
app.use(cors());
app.use(express.json());
app.use("/api/ai", aiFormatRecipe);
...
app.get("/api/recipes", async (req, res) => {
  ...
  if (!query) {
    ...
    try {
      ...
      if (!apiRes.ok) throw new Error("Random recipes failed");

      const data = await apiRes.json();
      const recipes = data.recipes.map(applyDefaults);
  ...
  try {
    const apiRes = await fetch(
      `https://api.spoonacular.com/recipes/complexSearch?query=${encodeURIComponent(query)}&number=10&addRecipeInformation=true&apiKey=${process.env.SPOONACULAR_KEY}`
    );

    if (!apiRes.ok) throw new Error("Search failed");

    const data = await apiRes.json();
    const recipes = data.results.map(applyDefaults);
